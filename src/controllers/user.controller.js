...
const updateUserAvatar = asyncHandler(async (req, res) => {
  ...
  const responce = await deleteOnCloudnary(req.user.avatar,"image");
...
const updateUserCoverImage = asyncHandler(async (req, res) => {
  ...
  await deleteOnCloudnary(user.coverimage,"image");
...
const getChannelProfile = asyncHandler(async (req, res) => {
  ...
  res
    .status(200)
    .json(
      new ApiResponce(200, channel[0], "User channel fetched successfuly.")
    );
});
  ...
  res
    ...
    .json(
      new ApiResponce(
        200,
        user[0].watchHistory,
        "WatchHistory fetched Succussfully"
      )
    );
});
